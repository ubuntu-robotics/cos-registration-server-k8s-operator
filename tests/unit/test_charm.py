...
class TestCharm(unittest.TestCase):
    ...
    def test_update_status(
        ...
        self.harness.charm._update_loki_alert_rules = Mock()
        self.harness.charm._update_prometheus_alert_rules = Mock()
        ...
        self.assertEqual(self.harness.charm._update_loki_alert_rules.call_count, 1)
        self.assertEqual(self.harness.charm._update_prometheus_alert_rules.call_count, 1)
    ...
    @patch("requests.get")
    def test_get_loki_alert_rules_files_from_db_success(self, mock_get):
        loki_alert = """group:
          - name: my-group
            alert: my-alert"""

        mock_get.return_value.json.return_value = [{"uid": "my_alert", "rules": loki_alert}]
        result = self.harness.charm._get_alert_rules_from_db("loki")
        self.assertEqual(
            result,
    ...
    @patch("requests.get")
    def test_update_loki_alert_rules_files_changed(self, mock_get):
        loki_alert = """group:
          - name: my-group
            alert: my-alert"""
        mock_get.return_value.json.return_value = [{"uid": "my_alert", "rules": loki_alert}]
        self.harness.charm._stored.loki_alert_rules_hash = ""
        self.harness.charm._update_loki_alert_rules()
        mock_get.assert_called_with(
            f"{self.harness.charm.internal_url}/api/v1/applications/loki/alert_rules/"
        )
    ...
    @patch("requests.get")
    def test_get_prometheus_alert_rules_files_from_db_success(self, mock_get):
        prometheus_alert = """group:
          - name: my-group
            alert: my-alert"""

        mock_get.return_value.json.return_value = [{"uid": "my_alert", "rules": prometheus_alert}]
        result = self.harness.charm._get_alert_rules_from_db("prometheus")
        self.assertEqual(
            result,
    ...
    @patch("requests.get")
    def test_update_prometheus_alert_rules_files_changed(self, mock_get):
        prometheus_alert = """group:
          - name: my-group
            alert: my-alert"""
        mock_get.return_value.json.return_value = [{"uid": "my_alert", "rules": prometheus_alert}]
        self.harness.charm._stored.prometheus_alert_rules_hash = ""
        self.harness.charm._update_prometheus_alert_rules()
        mock_get.assert_called_with(
            f"{self.harness.charm.internal_url}/api/v1/applications/prometheus/alert_rules/"
        )
    ...
    @patch("requests.get")
    def test_update_prometheus_alert_rules_files_not_changed(self, mock_get):
        prometheus_alert = """group:
          - name: my-group
            alert: my-alert"""
